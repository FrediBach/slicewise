...
Status: implementation started, 8 September 2026. The first Phase-0 kernel spike is implemented; the feasibility gate remains open. See [feasibility progress and measurements](./THREE_D_FEASIBILITY.md). The production application does not yet expose 3D mode. The remaining sections describe the intended implementation.
