...
The first integrated milestone now adds the 3D entry, lazy Three.js source viewport, reference views, orbit/pan/zoom, perspective/orthographic cameras, Studio/Inspect/Print display, explicit millimeter sizing and imported raw-unit selection, print rotation and bed placement. It uses shared Object deformation in a dedicated worker, preserves normalization provenance, scopes undo/redo, remembers bounded per-source session settings, rejects stale jobs and releases worker/GPU resources on exit. That initial milestone labeled the untreated source and kept preparation/export unavailable.

The next integrated step now adds compact 26 px buttons, shared planar slice controls/easing, fixed View depth capture and explicit alignment, selected-level overlays, experimental circular Inset/Emboss preparation, source/result comparison, cancellation/restart and separate geometry/manufacturing diagnostics. The kernel and audit limits remain unchanged. Preparation runs before print orientation/bed placement and audits the exact placed result again. Unsupported fields, incomplete audits and budget failures remain explicit rejections.

The resource milestone adds worker-local reuse of the physical base and exact planar contours across selection, treatment and placement edits. Retention is bounded to one base and one slice set within 64 MiB of typed buffers; oversized entries remain uncached. Source revisions and exact source arrays guard reuse, transferred replies own separate buffers, and preparation audits still run on every result. Automated checks cover transfer detachment, warm/cold result equivalence, invalidation and retention limits.

The built-in rounded cube, ripple sphere and diamond now use outward triangle winding. Their earlier negative signed volume was a source-generator defect. A default-resolution rounded cube at 80 mm, two height slices, 0.6 mm radius and 0.05 mm path approximation now passes source auditing, but its capsule tool is rejected for surface contacts in both Inset and Emboss. This remains an explicit, regression-covered preparation failure; fixing source orientation does not establish treatment feasibility.

The construction allowance is now 8,000 path vertices and 1,000,000 estimated capsule-construction triangles (both four times the initial cap), with 64 loops and final/input mesh and audit limits unchanged. An eight-slice default rounded cube has 2,048 exact path vertices and an estimated 532,480 construction triangles: it now reaches Boolean evaluation without approximation. At 80 mm and 0.6 mm radius, both operations are then rejected for degenerate result faces. Budget messages report complete workload counts; raising construction limits does not bypass final validation.

Latest refinement: eight exact rounded-cube slices now prepare successfully for Inset and Emboss. Generated-buffer cleanup removes exact coordinate duplicates and strictly collapsed faces, reports every change, and revalidates the result. Exact dyadic predicates resolve false-positive adjacent overlaps on thin generated triangles; nonadjacent proximity tests remain conservative. The current construction limits are 32,000 path vertices / 8,000,000 estimated triangles, with 500,000 mesh triangles and 20,000,000 intersection work units. Sixteen/32-slice exact cube trials now reach final auditing but retain surface-contact rejections. See the latest feasibility measurements below before treating larger limits as a performance guarantee.

The 100 mm Emboss follow-up resolves an additional Float32 rounding case: eight exact slices produced 16 real adjacent overlaps, despite passing at 80 mm. New projects now expose Result vertex cleanup with a 0.00001 mm default tolerance and an exact-only option. Generated Boolean vertices may merge only into fixed representatives within the selected bound; the measured maximum movement is disclosed and the resulting mesh is audited fully. Source/tool/placed cleanup behavior is otherwise unchanged. The reported case now passes, with maximum measured result-vertex movement about 0.00000853 mm. Regression coverage includes both sizes and treatments, plus preservation of the exact-only rejection.

Viewport visibility follow-up: changing a supported slice field, levels or selection now restores the new contour overlay after slices were hidden. Show/Hide slices labels make the action explicit; camera/placement preserve the choice. Comparison is scoped to the current prepared artifact, and slice failures are reported in the viewport as well as the panel. Fresh Height/Width/Depth/Custom Plane switches were verified to produce contours; this fixes stale display state rather than adding unsupported scalar fields.

Next: refine the captured geometry/profile failures and measure long-session resource behavior, then implement exact-artifact preparation/export gates and independently verified file interchange. The current coupled circular radius is an experimental parameter, not independent surface-normal width/depth. Preserve exact source/project revision association and reject incomplete validation. Independent width/depth, general surface-normal frames, file interchange, persistence and release validation remain open. The rectangular build volume is configurable per source during the session; reload persistence remains open. This milestone does not close Phase 0 or claim all Phase-1/2 gates complete. See [workspace verification](./TESTING.md) for the completed browser checks, 856 passing tests with an explicit timeout allowance, and remaining verification limits.
