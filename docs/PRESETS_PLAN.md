...
Status: implemented in the application. Versioned domain validation, full runtime capture/restore, original source retention, cancellation and full-workspace undo, native file sessions, folder management, handle persistence, and the Local/Examples panel are available. Four bundled examples cover all modes. Automated tests cover these integrations; native filesystem permissions/pickers and WebGL camera behavior still need manual browser verification.
...
The envelope schema is `src/lib/presets/preset-envelope.schema.json`; settings are validated by `workspace.ts` through `schema.ts`. All seven sections must be present and explicitly initialized or null where supported. No runtime values are inferred from the previous workspace. The current section codecs are version 1 with no historical production migrations yet. Numeric and enum constraints come from trusted application control catalogs; compound objects and mode unions have explicit schemas. Animation and Sequencer also pass their existing domain validators.

When adding authored state, update `inventory.ts`, its owning schema and capture/restore adapter, and round-trip fixtures. Add an explicit version migration with historical defaults when required fields or meaning change. Do not silently use today's defaults to repair old documents. Unknown compatible object fields survive edits. ID-bearing arrays merge by identity; positional compound arrays such as gradients reject unsupported fields to avoid silent data loss. Unknown active sections, required features, enum values, and unsupported versions block application with an error.

Initial transport budgets are 96 MiB per JSON file, 32 MiB per source asset, 64 MiB total decoded assets, 32 assets, 64 sections, 32 nesting levels, and 500,000 JSON nodes. Strings are limited separately from bounded embedded base64. Geometry preparation caps raw mesh input at two million triangles; subsequent render/3D budgets still apply. Folder scans stop at 5,000 entries, eight nested levels, or 256 MiB read. Assets never fetch arbitrary URLs. Nonembedded assets require exact byte length and SHA-256 matches.

`PresetFileSession` owns a digest baseline and write queue; reuse it for edits to the same file. `library.ts` persists only folder handles, not source content, and requests access through user gestures. The UI owns dirty state, permissions, file operation prompts, metadata, and local-copy attribution. File System Access has no universal atomic rename or compare-and-swap primitive: rename uses a verified copy followed by a source recheck and delete; saves detect prior external changes but cannot prevent a simultaneous external write in the final write window.

The static catalog contains UUID, name, description, tags, mode, filename, and SHA-256. To publish an example, save a complete self-contained preset, review its licensing and machine settings, add it under `public/presets/`, and update its catalog entry and digest. Run the catalog/runtime test. Thumbnails and a standalone legacy-snapshot export wizard are not part of this release; existing snapshots can be restored into an explicitly chosen current workspace and then saved as complete presets.
