...
`procedural-source.test.ts` compares generated frames with independent generation/normalization, checks both morph dimensions and exported paths, integer resolution and held animation seeds, immutable source reuse and bounded cache eviction. `procedural-runtime.test.tsx` verifies newly registered numeric/colour targets and generated animation edits without Config mesh replacement, including restoration on exit.
