...
- `gcode-3d-toolpaths.ts` defines the DOM-free machine-point and operation model for opt-in coordinated XYZ plotting. It converts already clipped and travel-ordered UUNA runs into constant-contact or bounded arc-length-resampled tapered strokes, then applies fixed-angle tip-offset compensation relative to Contact Z before serialization.
