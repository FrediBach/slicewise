...
- `slice-rays-settings.ts` shares optional planar-ray controls, defaults, compatibility checks, and saved-setting normalization. `slice-rays.ts` samples closed 3D slice loops by arc length, resolves outward directions by even/odd planar material parity, stops rays at other slice boundaries, and emits bounded deterministic straight segments with physical fade gaps. `contour-engine.ts` projects these after measuring the source contours, using background-aware depth visibility and the same explosion translation as the slice. Rays enter the common style/clipping/export pipeline; faded toolpaths preserve gaps during G-code travel optimization. Open sections and incompatible source/slicing modes bypass this stage.
