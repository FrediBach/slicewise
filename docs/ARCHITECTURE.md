...
- `contour-engine.ts` is the pure rendering core. It projects geometry, calculates scalar fields, slices triangles, chains line segments, performs visibility and silhouette work, applies output effects and SVG-only line-weight grouping, and returns SVG plus grouped centreline toolpaths. It must not read the DOM.
