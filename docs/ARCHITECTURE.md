...
- `sequencer-events.ts` is the shared live/export event compiler. It applies mute/solo state, exact lane timing, step lookup, and seeded probability in a stable order. `web-audio-engine.ts` is the browser-only adapter: audio context creation is deferred to `start()`, a 25 ms timer fills a 100 ms look-ahead window, delayed polls skip stale events, and pause/seek/stop cancel tracked sources. Its sample-free voices cover melodic oscillators plus synthesized kick, snare, hats, clap, and tom; explicit choke groups stop their preceding voice.
...
The versioned musical project autosaves as a small JSON envelope in browser-local storage through `sequencer-storage.ts`; it excludes mesh bytes and derived contour sources. Restore validates the complete melodic/drum discriminated union before the runtime accepts it. `midi-sequence-export.ts` writes a bounded format-1 Standard MIDI File using the transport PPQ. It calls the same `compileProjectEvents()` path as Web Audio, rounds absolute rational event positions rather than accumulating step errors, assigns percussion to General MIDI channel 10, and requires an explicit finite bar count.
