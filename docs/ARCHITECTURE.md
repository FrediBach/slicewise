...
- `solid-kernel.ts` is the internal Phase-0 3D feasibility adapter. It accepts explicit Z-up millimeter buffers, isolates Manifold import/union/difference and measurements, limits input/output complexity, and releases owned WASM solids in success and failure paths. Kernel acceptance is not full print validation. `three-d-feasibility.ts` supplies fixed analytic tools and sphere/box/torus/sheared-box fixtures; it does not extract production contours. `three-d-feasibility-worker.ts` loads the pinned WASM asset locally for the separate `tools/three-d-feasibility.html` developer page. The drawing application does not import these modules. See [the feasibility record](./THREE_D_FEASIBILITY.md) for commands, evidence, and open gates.
