...
- `projection.ts` owns deterministic camera bases, perspective blending, Klein↔Poincaré and optical lens warps, mesh projection, arbitrary world-point projection, and bounded adaptive projection of nonlinear polyline spans. Adaptive results distinguish valid, domain-clamped, and invalid samples; invalid samples split runs rather than leaking non-finite coordinates. Mesh contours, spirals, silhouettes, slice-exploded output, and imported SVG centrelines use this same pure projection path.
