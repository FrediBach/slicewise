...
- `projection.ts` owns deterministic camera bases, perspective blending, the discriminated None/Klein↔Poincaré/Möbius projection-warp stage, optical lens warps, mesh projection, arbitrary world-point projection, and bounded adaptive projection of nonlinear polyline spans. Möbius strength interpolates its bounded translation and rotation parameters from identity; points outside the disk use a transformed near-boundary direction with their original radial overflow restored. Projection results distinguish valid, domain-clamped, and invalid samples; invalid samples split runs rather than leaking non-finite coordinates. Mesh contours, spirals, silhouettes, slice-exploded output, and imported SVG centrelines use this same pure projection path.
- `scalar-fields.ts` defines the vertex-authoritative mesh scalar-field contract, constructs planar model/custom/camera fields and analytic spherical/cylindrical distance fields, assigns field-specific topology cache keys, and resolves compatibility for LFO, divergence, continuous spiral, and slice explosion. Planar fields retain a constant-direction fast path. Analytic wavefront fields provide off-vertex evaluation for adaptive intersection refinement and local normalized gradients for explosion; their centre/axis singularities return no gradient. Intrinsic fields interpolate vertex values without inventing an ambient evaluator.
...
- `mesh-geodesics.ts` computes deterministic multi-source shortest paths over the cached weighted edge graph. Its binary heap orders equal-distance work by vertex index, unreachable components remain at `Infinity`, and each solve returns an independent `Float64Array`. Directional seed selection uses a normalized model-space direction and the bounding-box centre, choosing the maximum centred projection with vertex index as the tie-break. These values are **surface graph distances**, not continuous exact geodesics.
- `contour-engine.ts` is the pure rendering core. It calculates scalar fields, delegates projection math to `projection.ts`, optionally warps slice fields with in-plane LFOs, adaptively subdivides nonlinear intersections, slices triangles, chains line segments, performs visibility and silhouette work, and returns SVG plus grouped centreline toolpaths. Output effects compose in a fixed order: geometry effects (including deterministic yarn cutting/curling) and clipping, path colour/weight styling, halftone styling, chromatic copies, map annotations, then document overlays. Mesh contours and imported SVG centrelines share those composition rules. It must not read the DOM.
...
Scalar fields with a non-empty `cacheKey` use a bounded set of at most eight 3D slice topologies per mesh. Every active planar decorator and contour-spacing parameter that changes topology participates in that key. The model-space `up`, `x`, `y`, and custom fields opt into this cache, including divergent fan slices whose virtual source axis is derived in model space. Orbiting can then reproject cached polylines instead of rescanning every triangle at every contour level. Camera-axis fields use an empty key and remain uncached because their vertex values change with the camera.

Mesh connectivity uses a separate lifetime cache in `mesh-topology.ts`. One installed mesh object maps to one immutable derived topology, reused by every surface-graph distance solve and later intrinsic field. Replacing the installed mesh naturally releases the old entry through the weak key. Distance buffers are not stored in this foundation cache; bounded per-seed distance caching belongs to the intrinsic-field integration that first needs it.
...
The hidden-line depth buffer deliberately continues to rasterize projected source triangles rather than adaptively tessellating every mesh face. The currently exposed radial transforms are bounded, and visibility uses a neighboring-pixel tolerance around the rasterized surface. Singular or horizon-based projections must revisit this decision before being exposed; the domain contract already splits invalid contour samples safely, but it does not by itself make a vertex-only depth mesh accurate across a projection horizon.
