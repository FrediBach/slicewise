...
- `scalar-fields.ts` defines the vertex-authoritative mesh scalar-field contract, constructs planar model/custom/camera fields, analytic spherical/cylindrical distance fields, and single-source intrinsic surface-graph distance fields. It assigns field-specific topology cache keys and resolves compatibility for LFO, divergence, continuous spiral, and slice explosion. Planar fields retain a constant-direction fast path. Analytic wavefront fields provide off-vertex evaluation for adaptive intersection refinement and local normalized gradients for explosion; their centre/axis singularities return no gradient. Geodesic fields interpolate finite vertex distances without inventing an ambient evaluator or gradient, and omit unreachable components.
...
Mesh connectivity uses a separate lifetime cache in `mesh-topology.ts`. One installed mesh object maps to one immutable derived topology, reused by every surface-graph distance solve and intrinsic field. Replacing the installed mesh naturally releases the old entry through the weak key. `scalar-fields.ts` additionally retains a least-recently-used insertion order of at most eight distance buffers per mesh seed vertex, preventing directional morphing from growing memory without bound.
