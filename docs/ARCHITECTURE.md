...
Animation controls use `animationmodechange` for the top-level Config/Animation switch, `animationcommand` for transport, timeline editing, and export cancellation, `animationstatechange` for runtime-to-React state, `animationstaterequest` for initial synchronization, and `animationexportprogress` for sequential render/encode status. In Animation mode, `slicer.ts` freezes a normal render-settings snapshot and intercepts morphable control input before the ordinary Config bindings can mutate runtime state. Quick playback renders use explicit evaluated snapshots and do not enter parameter history; leaving the mode restores the frozen Config snapshot.
