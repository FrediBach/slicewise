...
- `scalar-fields.ts` defines the vertex-authoritative mesh scalar-field contract, constructs planar model/custom/camera fields, assigns field-specific topology cache keys, and resolves compatibility for LFO, divergence, continuous spiral, and slice explosion. Planar fields retain a constant-direction fast path; analytic fields may provide off-vertex evaluation and local gradients; intrinsic fields interpolate vertex values without inventing an ambient evaluator.
