...
function RandomLock({ id, label }: RandomLockProps) {
  ...
  useEffect(() => {
    const update = (event: CustomEvent<CustomDetail>) => {
      ...
      document.dispatchEvent(
        new CustomEvent('randomlockchange', { detail: { id, locked: next, source: 'bulk' } }),
      );
    };
  ...
  const toggle = () => {
    ...
    setLocked(next);
    document.dispatchEvent(
      new CustomEvent('randomlockchange', { detail: { id, locked: next, source: 'individual' } }),
    );
  };
  return (
    <button
      type="button"
      className="random-lock"
      data-random-lock-id={id}
      aria-pressed={locked}
      aria-label={`${locked ? 'Unlock' : 'Lock'} ${label} randomization`}
      title={locked ? 'Include in randomization' : 'Exclude from randomization'}
      onClick={toggle}
    >
...
function RandomLockActions() {
  ...
  const apply = (mode: 'lock' | 'unlock') => {
    if (temporaryMode === mode) {
      document.dispatchEvent(
        new CustomEvent('randomlockbulk', { detail: { locks: previousLocks.current } }),
      );
      previousLocks.current = [];
      setTemporaryMode(null);
      return;
    ...
    if (!temporaryMode) {
      previousLocks.current = Array.from(
        document.querySelectorAll<HTMLButtonElement>('.random-lock[aria-pressed=true]'),
        (button) => button.dataset.randomLockId || '',
      );
    }
    document.dispatchEvent(
      new CustomEvent('randomlockbulk', { detail: { locked: mode === 'lock' } }),
    );
  ...
  return (
    <div className="lock-actions" aria-label="Bulk randomization locks">
      <Button
        type="button"
        variant="outline"
        className={`bulk-lock-button${temporaryMode === 'lock' ? ' active' : ''}`}
        aria-pressed={temporaryMode === 'lock'}
        title={
          temporaryMode === 'lock'
            ? 'Restore previous locks'
            : 'Temporarily lock every randomizable value'
        }
        onClick={() => apply('lock')}
      >
        <Lock size={12} />
        {temporaryMode === 'lock' ? 'Restore locks' : 'Lock all'}
      </Button>
      <Button
        type="button"
        variant="outline"
        className={`bulk-lock-button${temporaryMode === 'unlock' ? ' active' : ''}`}
        aria-pressed={temporaryMode === 'unlock'}
        title={
          temporaryMode === 'unlock'
            ? 'Restore previous locks'
            : 'Temporarily unlock every randomizable value'
        }
        onClick={() => apply('unlock')}
      >
        <LockOpen size={12} />
        {temporaryMode === 'unlock' ? 'Restore locks' : 'Unlock all'}
      </Button>
    </div>
  );
}
...
function ValueControl({
  ...
  const announceMorph = (dimension: number, active: boolean, nextValue: string | number) => {
    document.dispatchEvent(
      new CustomEvent('morphchange', {
        detail: { id, dimension, active, value: Number(nextValue) },
      }),
    );
  };
  const toggleMorph = () => {
    const secondEnabled = Boolean(
      (document.getElementById('morphSecondEnabled') as HTMLInputElement | null)?.checked,
    );
    const nextMode = (morphMode + 1) % (secondEnabled ? 3 : 2);
    const mainValue = Number(
      (document.getElementById(id) as HTMLInputElement | null)?.value ?? value,
    );
  ...
  useEffect(() => {
    const update = (event: CustomEvent<CustomDetail>) => {
      ...
      document.dispatchEvent(
        new CustomEvent('morphchange', { detail: { id, dimension, active: true, value: parsed } }),
      );
    };
    ...
    const secondDimension = (event: CustomEvent<CustomDetail>) => {
      ...
      document.dispatchEvent(
        new CustomEvent('morphchange', {
          detail: { id, dimension: 2, active: false, value: morphValueY },
        }),
      );
    };
  ...
  const morphInputs = (dimension: number, targetValue: number) => (
    <div className="control-inputs morph-inputs" data-dimension={dimension}>
      <span className="morph-axis" aria-hidden="true">
        {dimension === 1 ? 'X' : 'Y'}
      </span>
      <input
        type="range"
        id={`${id}Morph${dimension}`}
        min={min}
        max={max}
        step={step}
        value={targetValue}
        aria-label={`${label} morph ${dimension === 1 ? 'X' : 'Y'} target`}
        onChange={(event) => changeMorphValue(dimension, event.target.value)}
      />
      <span className={`value-field${unit ? ' has-unit' : ''}`}>
        <input
          type="number"
          id={`${id}Morph${dimension}N`}
          min={min}
          max={max}
          step={step}
          value={targetValue}
      ...
      <div className="control-label">
        ...
        {morphable && (
          <button
            ...
            aria-pressed={morphMode > 0}
            data-morph-dimension={morphMode}
            aria-label={`${label} morph mode: ${morphMode === 0 ? 'none' : morphMode === 1 ? 'X only' : 'X and Y'}`}
            title="Cycle morph mode: none, X, X + Y"
            onClick={toggleMorph}
          >
            <MorphIcon />
          </button>
...
type ColorControlProps = RandomLockProps & {
  defaultValue: string;
  swatchId: string;
  morphable?: boolean;
};
...
function ColorControl({ id, label, defaultValue, swatchId, morphable = true }: ColorControlProps) {
  ...
  const announce = (dimension: number, active: boolean, value: string) => {
    document.dispatchEvent(
      new CustomEvent('morphchange', { detail: { id, dimension, active, value } }),
    );
  };
  const toggle = () => {
    const secondEnabled = Boolean(
  ...
  useEffect(() => {
    const update = (event: CustomEvent<CustomDetail>) => {
      ...
      if (dimension === 1) {
        ...
        setTargetY(value);
        setTargetTextY(value);
      }
      document.dispatchEvent(
        new CustomEvent('morphchange', { detail: { id, dimension, active: true, value } }),
      );
    };
    document.addEventListener('randomizemorph', update);
    const secondDimension = (event: CustomEvent<CustomDetail>) => {
      ...
      document.dispatchEvent(
        new CustomEvent('morphchange', {
          detail: { id, dimension: 2, active: false, value: targetY },
        }),
      );
    };
    document.addEventListener('morphseconddimension', secondDimension);
      ...
      <span className="swatch" style={{ background: value }}>
        <input
          ...
          onChange={(event) => {
            ...
            setValidTarget(dimension, event.target.value);
          }}
        />
      </span>
      <input
        type="text"
        id={`${id}Morph${dimension}Hex`}
        value={text}
        spellCheck="false"
        aria-label={`${label} morph ${dimension === 1 ? 'X' : 'Y'} target hex value`}
        onChange={(event) => {
          setText(event.target.value);
          setValidTarget(dimension, event.target.value);
        }}
