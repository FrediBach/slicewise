...
describe("randomization locks", () => {
  it("toggles individually and restores lock state after a bulk override", async () => {
    ...
    expect(screen.getByRole("button", { name: "Unlock First randomization" })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: "Lock Second randomization" })).toHaveAttribute("aria-pressed", "false");
...
describe("ValueControl morph contract", () => {
  it("cycles through X and Y targets and publishes clamped values", async () => {
    ...
    act(() => document.dispatchEvent(new CustomEvent("morphseconddimension", { detail: { enabled: false } })));
    await waitFor(() => expect(screen.queryByRole("spinbutton", { name: "Scale morph Y target" })).not.toBeInTheDocument());
  ...
  it("restores both morph dimensions from parameter history", async () => {
    ...
    act(() => document.dispatchEvent(new CustomEvent("restoreparameters", {
    ...
    expect(screen.getByRole("button", { name: /Angle morph mode: X and Y/i })).toHaveAttribute("data-morph-dimension", "2");
  });
});
