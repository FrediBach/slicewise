...
export interface ThreeDRequest {
  ...
  settings: Partial<ObjectSettings>;
}
...
export function placeThreeDSource(
  ...
  const angles = project.rotation.map((v) => (v * Math.PI) / 180);
  const V = new Float32Array(mesh.V.length);
  for (let i = 0; i < V.length; i += 3) {
    let p = [0, 1, 2].map((k) => (mesh.V[i + k] - center[k]) * scale);
